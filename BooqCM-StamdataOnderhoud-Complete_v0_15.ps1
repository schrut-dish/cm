...
function Show-ProductMultiSelect {
    ...
    # Hashtable om de mapping tussen index en Product ID bij te houden
    $indexToIdMapping = @{}
    $allProducts = @()
    
    # Voeg alle products toe
    $itemIndex = 0
    foreach ($product in $Products) {
        ...
        # Check als het product pre-selected moet zijn
    ...
    $searchBox.Add_TextChanged({
        ...
        # Sla de checked items op voor herstel
        $checkedItems = @{}
        for ($i = 0; $i -lt $checkedListBox.Items.Count; $i++) {
            if ($checkedListBox.GetItemChecked($i)) {
                $prodId = $indexToIdMapping[$i]
                if ($prodId) {
                    $checkedItems[$prodId] = $true
                }
            }
        }
        ...
        foreach ($prod in $allProducts) {
            ...
            if ([string]::IsNullOrWhiteSpace($searchText)) {
                ...
                switch ($filterType) {
                    0 {  # Naam en ID
                        if ($prod.Name.ToLower().Contains($searchText) -or 
                            $prod.Id.ToLower().Contains($searchText)) {
                            $matchFound = $true
                        }
                    }
                    1 {  # Alleen Naam
                    ...
                    2 {  # Alleen ID
                        if ($prod.Id.ToLower().Contains($searchText)) {
                            $matchFound = $true
                        }
                    }
                }
            ...
            if ($matchFound) {
                ...
                # Herstel checked status op basis van product ID
    ...
    # Update count bij selectie wijziging
    $checkedListBox.Add_ItemCheck({
        # Gebruik BeginInvoke om na de check te updaten
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        $selectedIds = @()
        foreach ($index in $checkedListBox.CheckedIndices) {
            # Haal de Product ID op uit de mapping
            $prodId = $indexToIdMapping[$index]
            if ($prodId) {
                $selectedIds += $prodId
            }
        }
...
function Select-Product {
    ...
    switch ($choice) {
        "1" {
            ...
            $selected = Show-SelectionMenu -Title "Selecteer Product uit sessie" -Items $sessionProducts -DisplayProperty "id" -IdProperty "id" -AllowCancel $true
            if ($selected) {
                return $selected.id
            }
            return Select-Product -Session $Session -Purpose $Purpose -AllowMultiple $AllowMultiple
        ...
        "2" {
            ...
            if ($products.Count -gt 20) {
                ...
                if ($selectedIds -and $selectedIds.Count -gt 0) {
                    ...
                    # Voeg toe aan sessie voor later gebruik (BUITEN de form closure)
                    foreach ($id in $selectedIds) {
                        if (-not $Session.CreatedProducts) {
                            $Session | Add-Member -NotePropertyName "CreatedProducts" -NotePropertyValue @() -Force
                        }
                    ...
                    # Als AllowMultiple false is, return alleen de eerste
                    if (-not $AllowMultiple -and $selectedIds.Count -gt 0) {
                ...
                $selected = Show-SelectionMenu -Title "Selecteer Product uit PIM API" -Items $products -DisplayProperty "name" -IdProperty "id" -AllowCancel $true
                if ($selected) {
                    # Voeg toe aan sessie voor later gebruik
                    if (-not $Session.CreatedProducts) {
                        $Session | Add-Member -NotePropertyName "CreatedProducts" -NotePropertyValue @() -Force
                    }
                    if ($selected.id -notin $Session.CreatedProducts) {
                        $Session.CreatedProducts += $selected.id
                    }
                    return $selected.id
                }
                return Select-Product -Session $Session -Purpose $Purpose -AllowMultiple $AllowMultiple
...
function New-Promotion {
    ...
    Clear-Host
    Write-Host "`n+==============================================================" -ForegroundColor Cyan
    Write-Host "|              NIEUWE PROMOTIE AANMAKEN                        |" -ForegroundColor Cyan
    Write-Host "+==============================================================+" -ForegroundColor Cyan
    Write-Host ""
    
    # Stap 1: Basis informatie
    Write-Host "STAP 1: BASIS INFORMATIE" -ForegroundColor Yellow
    Write-Host "=========================" -ForegroundColor Yellow
    
    $promotionId = Read-InputWithValidation -Prompt "`nGeef een unieke ID voor de Promotie" -Type "String" -Mandatory $true
    $name = Read-InputWithValidation -Prompt "Naam van de Promotie" -Type "String" -Mandatory $true
    
    # Stap 2: Promotie type
    Write-Host "`nSTAP 2: PROMOTIE TYPE" -ForegroundColor Yellow
    Write-Host "=====================" -ForegroundColor Yellow
    Write-Host "1. TICKET_DISCOUNT - Korting op hele bon"
    Write-Host "2. QUANTITY_DISCOUNT - Korting op aantal"
    Write-Host "3. COMBI_DEAL - Combinatie deal"
    
    $typeChoice = Read-Host "`nKeuze"
    ...
    # Stap 3: Effect (korting)
    Write-Host "`nSTAP 3: KORTING EFFECT" -ForegroundColor Yellow
    Write-Host "======================" -ForegroundColor Yellow
    Write-Host "1. NEW_AMOUNT - Nieuw bedrag"
    Write-Host "2. DISCOUNT_AMOUNT - Korting in vaste bedrag"
    Write-Host "3. DISCOUNT_PERCENTAGE - Korting in percentage"
    
    $effectChoice = Read-Host "`nKeuze"
    $effectType = switch ($effectChoice) {
        "1" { "NEW_AMOUNT" }
        "2" { "DISCOUNT_AMOUNT" }
    ...
    $effectValue = Read-InputWithValidation -Prompt "Waarde (bijv. 5 voor 5%, of 10 voor EUR 10)" -Type "Decimal" -Mandatory $true
    ...
    # Stap 4: Product selectie (voor QUANTITY_DISCOUNT en COMBI_DEAL)
    $productConditions = $null
    if ($type -eq "QUANTITY_DISCOUNT" -or $type -eq "COMBI_DEAL") {
        Write-Host "`nSTAP 4: PRODUCT CONDITIE" -ForegroundColor Yellow
        Write-Host "========================" -ForegroundColor Yellow
        
        $productId = Select-Product -Session $Session -Purpose "Selecteer product voor promotie"
        
        if ($productId) {
            $minQuantity = Read-InputWithValidation -Prompt "Minimum aantal" -Type "Int" -Mandatory $false -DefaultValue "1"
            ...
            $productConditions = @(
                @{
                    ...
                    minQuantity = if ($minQuantity) { [int]$minQuantity } else { 1 }
                }
            )
        }
    }
    
    # Stap 5: Ticket conditie (voor TICKET_DISCOUNT)
    $ticketCondition = $null
    if ($type -eq "TICKET_DISCOUNT") {
        Write-Host "`nSTAP 4: TICKET CONDITIE (optioneel)" -ForegroundColor Yellow
        Write-Host "====================================" -ForegroundColor Yellow
        
        $addTicketCondition = Read-InputWithValidation -Prompt "Wilt u een minimum bedrag instellen? (ja/nee)" -Type "String" -Mandatory $false -DefaultValue "nee"
        
        if ($addTicketCondition -match "^(ja|yes|j|y)$") {
            $minAmount = Read-InputWithValidation -Prompt "Minimum bedrag" -Type "Decimal" -Mandatory $true
            
            $ticketCondition = @{
                minimumAmount = [decimal]$minAmount
            }
        }
    }
    
    # Stap 6: Availability
    $stepNumber = if ($type -eq 'TICKET_DISCOUNT') { '5' } else { '5' }
    Write-Host "`nSTAP ${stepNumber}: BESCHIKBAARHEID" -ForegroundColor Yellow
    Write-Host "===========================" -ForegroundColor Yellow
    
    $availabilityIds = Select-Availability -Session $Session -Purpose "Selecteer Availability/Availabilities voor promotie" -AllowMultiple $true
    
    # Stap 7: Customers (optioneel)
    $stepNumber = if ($type -eq 'TICKET_DISCOUNT') { '6' } else { '6' }
    Write-Host "`nSTAP ${stepNumber}: KLANTEN (optioneel)" -ForegroundColor Yellow
    Write-Host "===============================" -ForegroundColor Yellow
    
    $customerIds = Select-Customer -Session $Session -Purpose "Selecteer klanten voor promotie (optioneel)" -AllowMultiple $true
    
    # Bouw de request body op
    $body = @{
        name = $name
        type = $type
        effect = $effect
    ...
    if ($productConditions) {
        $body['productConditions'] = $productConditions
    }
    
    if ($ticketCondition) {
        $body['ticketCondition'] = $ticketCondition
    }
    
    if ($availabilityIds) {
        # Zorg ervoor dat availability altijd een array is, zelfs met 1 item
        if ($availabilityIds -is [array]) {
            $body['availability'] = $availabilityIds
        } else {
            $body['availability'] = @($availabilityIds)
        }
    }
    ...
    # Toon samenvatting
    Write-Host "`n==============================================================" -ForegroundColor Cyan
    Write-Host "SAMENVATTING PROMOTIE" -ForegroundColor Cyan
    Write-Host "==============================================================" -ForegroundColor Cyan
    Write-Host "ID: $promotionId" -ForegroundColor White
    Write-Host "Naam: $name" -ForegroundColor White
    Write-Host "Type: $type" -ForegroundColor White
    Write-Host "Effect: $effectType - $effectValue" -ForegroundColor White
    
    if ($productConditions) {
        Write-Host "Product conditie: Ja (product ID: $($productConditions[0].what.productId))" -ForegroundColor White
    }
    
    if ($ticketCondition) {
        Write-Host "Ticket conditie: Minimum bedrag EUR $($ticketCondition.minimumAmount)" -ForegroundColor White
    }
    
    if ($availabilityIds) {
        Write-Host "Availabilities: $($availabilityIds.Count) geselecteerd" -ForegroundColor White
    }
    
    if ($customerIds -and $customerIds.Count -gt 0) {
        Write-Host "Customers: $($customerIds.Count) geselecteerd" -ForegroundColor White
        if ($DebugMode) {
            Write-Host "  IDs: $($customerIds -join ', ')" -ForegroundColor Gray
        }
        Write-Host "  Let op: Customer IDs moeten bestaan in de API" -ForegroundColor Yellow
    }
    
    Write-Host "==============================================================" -ForegroundColor Cyan
    
    $confirm = Read-InputWithValidation -Prompt "`nPromotie aanmaken? (ja/nee)" -Type "String" -Mandatory $true
    
    if ($confirm -notmatch "^(ja|yes|j|y)$") {
        Write-Host "Promotie niet aangemaakt." -ForegroundColor Yellow
        Read-Host "`nDruk op Enter om door te gaan"
        return $null
    }
    
    Write-Host "`nPromotie wordt aangemaakt..." -ForegroundColor Yellow
    
    if ($DebugMode) {
        Write-Log "=== PROMOTION REQUEST BODY ===" -level DEBUG
        Write-Log "Availability count: $($body['availability'].Count)" -level DEBUG
        Write-Log "Availability IDs: $($body['availability'] -join ', ')" -level DEBUG
        if ($body['customerCondition']) {
            Write-Log "Customer count: $($body['customerCondition']['customers'].Count)" -level DEBUG
            Write-Log "Customer IDs: $($body['customerCondition']['customers'] -join ', ')" -level DEBUG
        }
    }
    
    try {
        $result = Invoke-CMApi -Endpoint "promotions/$promotionId" -Method "PUT" -Body $body
        Write-Host "`n[OK] Promotie succesvol aangemaakt!" -ForegroundColor Green
        
        Add-ToHistory -Session $Session -Action "Create" -EntityType "Promotion" -EntityId $promotionId -Parameters @{ Body = $body } -Result $result
        
        Read-Host "`nDruk op Enter om door te gaan"
        return $promotionId
    }
    catch {
        Write-Host "`n[FOUT] Fout bij aanmaken promotie: $_" -ForegroundColor Red
        Write-Host "Details: $($_.Exception.Message)" -ForegroundColor Red
        
        # Geef suggesties op basis van de error
        if ($_ -match "Validation failed") {
            Write-Host "`nMogelijke oorzaken:" -ForegroundColor Yellow
            if ($body['availability']) {
                Write-Host "  1. Een Availability ID bestaat niet (controleer: $($body['availability'] -join ', '))" -ForegroundColor Gray
            }
            if ($body['productConditions']) {
                $productIds = $body['productConditions'] | ForEach-Object { $_.what.productId }
                Write-Host "  2. Een Product ID bestaat niet of is ongeldig" -ForegroundColor Gray
                Write-Host "     Geselecteerd: $($productIds -join ', ')" -ForegroundColor Gray
                Write-Host "     TIP: Controleer of dit product bestaat in de PIM API" -ForegroundColor Yellow
            }
            if ($body['customerCondition']) {
                Write-Host "  3. Een Customer ID bestaat niet of is ongeldig" -ForegroundColor Gray
                Write-Host "     Geselecteerd: $($body['customerCondition']['customers'] -join ', ')" -ForegroundColor Gray
            }
            Write-Host "  4. Een verplicht veld ontbreekt of heeft verkeerd formaat" -ForegroundColor Gray
        }
        
        Write-Host "`nWat wilt u doen?" -ForegroundColor Cyan
        Write-Host "1. Opnieuw proberen (nieuwe invoer)" -ForegroundColor Green
        Write-Host "2. Terug naar hoofdmenu" -ForegroundColor Red
        
        $choice = Read-Host "`nKeuze"
        
        if ($choice -eq "1") {
            return New-Promotion -Session $Session
        }
...
function Show-MainMenuGUI {
    ...
    $btnAddPromotion.Add_Click({
        ...
        Show-MainMenuGUI -Session $Session
    })
    ...
    $btnLoadSession.Add_Click({
        ...
        if ($newSession) {
            ...
            Show-MainMenuGUI -Session $newSession
        } else {
            Show-MainMenuGUI -Session $Session
        }
    })
